...
public final class NotFoundException extends RuntimeException {
  ...
  public NotFoundException(Iterable<Message> messages) {
    this.messages = ImmutableSet.copyOf(messages); 
    initCause(Errors.getOnlyCause(this.messages));
  }

  /** Returns a copy of this configuration exception with the specified partial value. */
  public NotFoundException withPartialValue(Object partialValue) {
    checkState(this.partialValue == null, "Can't clobber existing partial value %s with %s", this.partialValue, partialValue);
    NotFoundException result = new NotFoundException(messages);
  ...
  @Override public String getMessage() {
    return Errors.format("eXo social client data errors", messages);
  }
