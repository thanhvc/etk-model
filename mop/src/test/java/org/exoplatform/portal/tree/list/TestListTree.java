...
public class TestListTree extends TestCase {
  ...
  /*
   * public void testInsertDuplicate() { IntegerTree root = tree("", 0,
   * tree("a", 1)); assertChildren(root, 1); assertAllChildren(root, 1); // try
   * { root.insertAt(0, tree("a", 2)); fail(); } catch (IllegalArgumentException
   * ignore) { assertAllChildren(root, 1); } }
   */
  ...
  /*
   * public void testRename() { IntegerTree root = tree("", 0, tree("a", 1),
   * tree("b", 2), tree("c", 3)); assertAllChildren(root, 1, 2, 3);
   * assertAllChildren(root, "a", "b", "c"); // root.rename("a", "a");
   * assertAllChildren(root, 1, 2, 3); assertAllChildren(root, "a", "b", "c");
   * // root.rename("a", "d"); assertAllChildren(root, 1, 2, 3);
   * assertAllChildren(root, "d", "b", "c"); } public void
   * testRenameWithNoChildren() { IntegerTree root = tree("", 0, (IntegerTree[])
   * null); assertFalse(root.hasTrees()); // try { root.rename("a", "b");
   * fail(); } catch (IllegalStateException e) { assertFalse(root.hasTrees()); }
   * } public void testRenameWithNonExisting() { IntegerTree root = tree("", 0,
   * tree("a", 1), tree("b", 2), tree("c", 3)); assertAllChildren(root, 1, 2,
   * 3); assertAllChildren(root, "a", "b", "c"); // try { root.rename("d", "e");
   * fail(); } catch (IllegalArgumentException e) { assertAllChildren(root, 1,
   * 2, 3); assertAllChildren(root, "a", "b", "c"); } } public void
   * testRenameWithExisting() { IntegerTree root = tree("", 0, tree("a", 1),
   * tree("b", 2), tree("c", 3)); assertAllChildren(root, 1, 2, 3);
   * assertAllChildren(root, "a", "b", "c"); // try { root.rename("a", "c");
   * fail(); } catch (IllegalArgumentException e) { assertAllChildren(root, 1,
   * 2, 3); assertAllChildren(root, "a", "b", "c"); } }
   */

  /*
   * public void testGetByIndexWithNoChildren() { IntegerTree root = tree("", 0,
   * (IntegerTree[]) null); // try { root.get(0); fail(); } catch
  ...
  /*
   * @Override
   * @SuppressWarnings("unchecked") public void testListIteratorSet() {
   * list.add((E) "1"); list.add((E) "2"); list.add((E) "3"); list.add((E) "4");
   * list.add((E) "5"); ListIterator<E> it = list.listIterator();
   * assertEquals("1", it.next()); it.set((E) "a"); assertEquals("a",
   * it.previous()); it.set((E) "A"); assertEquals("A", it.next());
   * assertEquals("2", it.next()); it.set((E) "B"); assertEquals("3",
   * it.next()); assertEquals("4", it.next()); it.set((E) "D");
   * assertEquals("5", it.next()); it.set((E) "E");
